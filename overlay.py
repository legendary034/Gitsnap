...
class ActionOverlay:
    ...
    """
    def __init__(self, parent, img, _x, _y, on_copy, on_upload, is_video=False, video_path=None):
        self.img = img
        ...
        btn_frame.pack()
        if self.is_video:
            tk.Label(btn_frame, text="Video Ready", bg='white',
                     font=("Helvetica", 9, "bold")).pack(side=tk.TOP, pady=(0, 4))
            tk.Button(btn_frame, text="Save Locally",
                      command=lambda: [self.window.destroy(), self.save_video(on_copy)]
                      ).pack(side=tk.LEFT, padx=2)
            tk.Button(btn_frame, text="Upload",
                      command=lambda: [self.window.destroy(), on_upload(None, self.video_path)]
                      ).pack(side=tk.LEFT, padx=2)
        else:
            tk.Button(btn_frame, text="Copy",
                      command=lambda: [self.window.destroy(), on_copy(self.img)]
                      ).pack(side=tk.LEFT, padx=2)
            tk.Button(btn_frame, text="Upload",
...
def show_action_overlay(parent, img, _x, _y, on_copy, on_upload, is_video=False, video_path=None):
    """
    Convenience function to create and show an ActionOverlay.
    """
    ActionOverlay(parent, img, _x, _y, on_copy, on_upload, is_video, video_path)
